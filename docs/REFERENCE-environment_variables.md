...
DEFAULT_TIMEZONE_OFFSET           | Used for Direct Warehouse Querying for Contact List Setting. Timezone offset is a combination of the actual offset from UTC time (for example, EST offset is -5 and PST offset is -8) and whether that timezone respects DST. This value is used to determine whether a contact can be contacted within textable hours. Example value is "-5_1" for EST and "-6_1" for CST. This default timezone offset value is only used if this data is not included in the original SQL query results. This value sets to "-5_1" if this environment variable is not set AND timezone_offset is not included as a field in the original SQL query results. 
