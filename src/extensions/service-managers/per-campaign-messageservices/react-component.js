...
export class CampaignConfig extends React.Component {
  ...
  constructor(props) {
    ...
    this.state = {
      ...
      inventoryPhoneNumberCounts:
        props.serviceManagerInfo.data.inventoryPhoneNumberCounts
    };
  }
  ...
  setSuppressedAreaCodes = () => {
    const { isStarted } = this.props.campaign;
    const {
      phoneNumberCounts,
      contactsAreaCodeCounts,
      contactsPerPhoneNumber,
      inventoryPhoneNumberCounts: inventoryCounts
  ...
  renderPhoneNumbers() {
    const {
      ...
      showOnlySelected,
      suppressedAreaCodes,
      inventoryPhoneNumberCounts: reservedNumbers
    } = this.state;
    const { isStarted, contactsCount } = this.props.campaign;
    const {
      contactsPerPhoneNumber,
      phoneNumberCounts
    } = this.props.serviceManagerInfo.data;
    ...
    let areaCodes = _.orderBy(
      phoneNumberCounts
        .map(phoneNumber => {
          const foundReserved = reservedNumbers.find(
    ...
    const getAssignedCount = areaCode => {
      const inventory = this.state.inventoryPhoneNumberCounts;
      return (
    ...
    const assignAreaCode = areaCode => {
      ...
      this.handleChange({ inventoryPhoneNumberCounts });
  ...
  renderSubtitle = () => {
    const { contactsPerPhoneNumber } = this.props.serviceManagerInfo.data;
    return (
      <div>
  ...
  renderSearch() {
    const { isStarted } = this.props.campaign;
    const { phoneNumberCounts } = this.props.serviceManagerInfo.data;
  ...
  renderAreaCodeTable() {
    const { inventoryPhoneNumberCounts: reservedNumbers } = this.state;
    const assignedNumberCount = this.getTotalNumberCount(reservedNumbers);
    const { isStarted, contactsCount } = this.props.campaign;
    const {
      inventoryPhoneNumberCounts: inventoryCounts,
    ...
    const autoAssignRemaining = () => {
      ...
      const availableAreaCodes = _.flatten(
        this.props.serviceManagerInfo.data.phoneNumberCounts
          .filter(
            phoneNumber =>
  ...
  renderContactsAreaCodesTable() {
    ...
    const { contactsCount } = this.props.campaign;
    const {
      contactsAreaCodeCounts,
      contactsPerPhoneNumber
    } = this.props.serviceManagerInfo.data;
    ...
    ).reduce((arr, [state, count]) => {
      // show all states with significant needs
      return count / contactsPerPhoneNumber >= 0.4
        ? [
            ...arr,
            {
              state,
  ...
  render() {
    ...
    const { inventoryPhoneNumberCounts: reservedNumbers } = this.state;
    const assignedNumberCount = this.getTotalNumberCount(reservedNumbers);
    const { contactsCount, isStarted } = this.props.campaign;
    const { isRendering, hasReset } = this.state;
    const numbersNeeded = Math.ceil(contactsCount / contactsPerPhoneNumber);
    ...
    return (
      <GSForm
        ...
        value={{ inventoryPhoneNumberCounts: reservedNumbers }}
        onChange={this.handleChange}
        onSubmit={() => {
          if (assignedNumberCount === numbersNeeded || hasReset) {
            this.props.onSubmit({
              inventoryPhoneNumberCounts: this.state.inventoryPhoneNumberCounts
            });
        ...
        {numbersNeeded <= maxNumbersPerCampaign ? (
          <div>
            ...
            <Form.Submit
              ...
              disabled={
                isStarted ||
                (assignedNumberCount !== numbersNeeded && !hasReset) ||
                isRendering
              }
