...
const assignmentHashKey = id =>
  `${process.env.CACHE_PREFIX || ""}assignment-${id}`;
...
const loadCampaignAssignments = async campaign => {
  if (r.redis) {
    ...
    const assignments = await assignmentQuery().where(
      "campaign_id",
      campaign.id
    );
...
const assignmentCache = {
  ...
  userInflightCounts: async campaignId => {
    ...
    return textersWithInFlights.map(obj => ({
      id: obj.userId,
