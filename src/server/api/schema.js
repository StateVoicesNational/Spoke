...
const rootMutations = {
  RootMutation: {
    ...
    editUser: async (_, { organizationId, userId, userData }, { user }) => {
      ...
      if (!userRes || !userRes.length) {
        ...
        if (userData) {
          ...
          await cacheableData.assignment.clearUserAssignments(
            organizationId,
            userId
          );
          userData = {
      ...
      const contacts = contactIds.map(async contactId => {
        ...
        if (contact && Number(contact.assignment_id) === Number(assignmentId)) {
          ...
          if (contact.is_opted_out) {
            ...
            cacheableData.assignment
              .optOutContact(assignmentId, contactId, [contact.timezone_offset])
              .then(() => 1);
          }
          return contact;
        }
    ...
    findNewCampaignContact: async (
      ...
      return {
        found: Boolean(
          await cacheableData.assignment.findNewContacts(
            assignment,
            campaign,
            organization,
            numberContacts
    ...
    sendMessage: async (
      ...
      if (
        Number(contact.assignment_id) !== Number(message.assignmentId) ||
        campaign.is_archived
      ) {
        console.error(
          "sendMessage WRONG ASSIGNMENT",
