...
const findGraphqlContactSelection = operation => {
  ...
  if (
    operation.name &&
    operation.name.value === "assignment" &&
    operation.selectionSet
  ) {
    if (operation.selectionSet) {
      const [contactQuery] = operation.selectionSet.selections.filter(
        sel => sel.name.value === "contacts"
      );
      if (contactQuery) {
        if (contactQuery.selectionSet && contactQuery.selectionSet.selections) {
          return {
            found: true,
  ...
  if (operation.selectionSet) {
    const [foundSomething] = operation.selectionSet.selections
      .map(sel => {
        if (sel.selectionSet) {
...
export const resolvers = {
  Assignment: {
    ...
    campaign: async (assignment, _, { loaders }) =>
      assignment.campaign || loaders.campaign.load(assignment.campaign_id),
    ...
    contacts: async (
      ...
      const justNeedContactIds = graphqlQueryJustContactIds(
        graphqlRequest.operation
      );
