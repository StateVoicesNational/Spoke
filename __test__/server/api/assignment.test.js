...
describe('test getContacts builds queries correctly', () => {
  ...
  it('works with: no contacts filter', () => {
    ...
    expect(query.toString()).toBe(
      'select * from \"campaign_contact\" where \"assignment_id\" = 1 and \"is_opted_out\" = false order by message_status DESC, updated_at'
    )
  }) // it
  ...
  it('works with: contacts filter, include past due, message status', () => {
    ...
    expect(query.toString()).toBe(
       "select * from \"campaign_contact\" where \"assignment_id\" = 1 and \"is_opted_out\" = false and \"message_status\" in ('needsResponse', 'needsMessage') order by message_status DESC, updated_at"
    )
  }) // it
  ...
  it('works with: contacts filter, exclude past due, campaign is past due, message status needsMessage', () => {
    ...
    expect(query.toString()).toBe(
       "select * from \"campaign_contact\" where \"assignment_id\" = 1 and \"is_opted_out\" = false and \"message_status\" = '' order by message_status DESC, updated_at"
    )
  }) // it
  ...
  it('works with: contacts filter, exclude past due, message status one other', () => {
    ...
    expect(query.toString()).toBe(
      "select * from \"campaign_contact\" where \"assignment_id\" = 1 and \"is_opted_out\" = false and \"message_status\" in ('convo') order by message_status DESC, updated_at"
    )
  }) // it
  ...
  it('works with: contacts filter, exclude past due, message status multiple other', () => {
    ...
    expect(query.toString()).toBe(
       "select * from \"campaign_contact\" where \"assignment_id\" = 1 and \"is_opted_out\" = false and \"message_status\" in ('convo', 'messageReceived') order by message_status DESC, updated_at"
    )
  }) // it
  ...
  it('works with: contacts filter, exclude past due, no message status, campaign is past due', () => {
    ...
    expect(query.toString()).toBe(
      'select * from "campaign_contact" where "assignment_id" = 1 and \"is_opted_out\" = false and "message_status" = \'needsResponse\' order by message_status DESC, updated_at'
    )
  }) // it
  ...
  it('works with: forCount, contacts filter, exclude past due, no message status, campaign not past due', () => {
    ...
    expect(query.toString()).toBe(
      'select * from "campaign_contact" where "assignment_id" = 1 and \"is_opted_out\" = false and "message_status" in (\'needsResponse\', \'needsMessage\')'
    )
  }) // it
}) // describe
...
describe('test getContacts timezone stuff only', () => {
  ...
  it('returns the correct query -- in default texting hours, with valid_timezone == true', () => {
    ...
    expect(query.toString()).toMatch(
       "select * from \"campaign_contact\" where \"assignment_id\" = 1 and \"is_opted_out\" = false and \"timezone_offset\" in ('-5_1', '') and \"message_status\" in ('needsResponse', 'needsMessage') order by message_status DESC, updated_at"
    )
  }) // it
  ...
  it('returns the correct query -- in default texting hours, with valid_timezone == false', () => {
    ...
    expect(query.toString()).toMatch(
       "select * from \"campaign_contact\" where \"assignment_id\" = 1 and \"is_opted_out\" = false and \"timezone_offset\" in ('-4_1') and \"message_status\" in ('needsResponse', 'needsMessage') order by message_status DESC, updated_at"
    )
  }) // it
